...
<div class="row">
    ...
    <script>
        ...
        window.addEventListener('printImage', (event) => {
            const { type, url, name, id, unit, number } = event.detail;
            
            // الحصول على عنصر الـ canvas والسياق
            ...
            img.onload = () => {
                // تعيين حجم الـ canvas ليكون نفس حجم الصورة
                ...
                JsBarcode(barcodeCanvas, number, { format: 'CODE128', width: 7, height: 200 });

                const barcodeX = canvas.width - barcodeCanvas.width -150; // الموضع الأفقي للباركود
                const barcodeY = 450; // الموضع العمودي للباركود
                ctx.drawImage(barcodeCanvas, barcodeX, barcodeY);

                // تعيين خصائص النص
                ctx.font = '50px NotoKufiArabic'; // حجم الخط 40 بكسل
                ctx.fillStyle = 'black'; // لون النص أبيض
                ctx.strokeStyle = 'black'; // لون الحد أسود
                ctx.lineWidth = 1; // عرض الحد
                ctx.textAlign = 'right'; // النص على اليمين
                ctx.textBaseline = 'top'; // النص في الأعلى

                // تحديد النصوص والموقع
                const lines = [
                    'الاسم: ' + name,
                    'رقم الهوية: ' + id,
                    'رقم المخيم: ' + unit,
                ...
                const x = canvas.width - 70; // الموضع الأفقي (على اليمين مع هامش 10 بكسل)
                const y = barcodeY + barcodeCanvas.height + 80; // الموضع العمودي (في الأعلى مع هامش 10 بكسل)
                const lineHeight = 100; // ارتفاع السطر 50 بكسل

                lines.forEach((line, index) => {
                    // حساب الموضع العمودي لكل سطر
            ...
            const modifiedImageURL = canvas.toDataURL(format);

                // استخدام printJS لطباعة الصورة المعدلة

                if( type === 'print' ){
                    printJS({
                        printable: modifiedImageURL,
                        type: 'image',
                        header: null,
                        footer: null,
                        honorMarginPadding: false,
                        honorColor: false,
                        style: 'body { margin: 0; padding: 0; } .card { width: 100%; height: 100%; }'
                    });
                }else{
                    const downloadLink = document.createElement('a');
                    downloadLink.href = modifiedImageURL;
                    downloadLink.download = `${name}_${number}.${extension}`;
