<div>
    @isset ( $swapLink )
        <a href="javascript:void(0)" wire:click="startSwapAddress({{ $swapLink }})"
            wire:loading.attr="disabled" 
            wire:target="startSwapAddress({{ $swapLink }})"
            title="تبديل السكن مع حاج آخر في نفس المخيم">
            <i class="fa-solid fa fa-exchange me-2" wire:loading.remove wire:target="startSwapAddress({{ $swapLink }})"></i>
            <i class="fa fa-spinner fa-spin" wire:loading wire:target="startSwapAddress({{ $swapLink }})"></i>
        </a>
    @endif
